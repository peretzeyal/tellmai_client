...
{	
	...
	private void handleBufferReading(){
		Log.d(TAG, "read record buffer");
	...
	private void waitForBufferReadingPeriod(){
		Thread thread = new Thread(new Runnable() {
			
			@Override
			public void run() {
				double startproctime = System.currentTimeMillis();
				double endproctime = 0;
				while(recordState){//state == State.RECORDING){
					endproctime = System.currentTimeMillis();
					Log.d(TAG, "check framePeriod time");
					if (startproctime + framePeriod >= startproctime){
						endproctime = System.currentTimeMillis();
						handleBufferReading();
	...
	public ExtAudioRecorder(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat)
	{
		try
		{
			...
			{ // RECORDING_UNCOMPRESSED
				...
				if (channelConfig == AudioFormat.CHANNEL_IN_MONO)//AudioFormat.CHANNEL_CONFIGURATION_MONO)
				{
					nChannels = 1;
				}
				else
				...
				Log.w(TAG, "set framePeriod " + Integer.toString(framePeriod));
				Log.w(TAG, "get bufferSize " + Integer.toString(bufferSize));
				/*				int minBufferSize = AudioRecord.getMinBufferSize(8000, AudioFormat.CHANNEL_IN_MONO,
                        AudioFormat.ENCODING_PCM_16BIT);
				Log.w(TAG, "set minBufferSize " + Integer.toString(minBufferSize));
				...
				Log.d(TAG, "updateListener created ");
